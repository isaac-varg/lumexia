...
const ByStatusPanel = ({ requests, statuses, priorities }: { requests: RequestForDashboard[], statuses: RequestStatus[], priorities: RequestPriority[] }) => {
  ...
  const canceledStatus = statusCounts[statusCounts.findIndex((s) => s.id === requestStatuses.requestCancelledDuplicateRequest)]
    ...
    <div className='flex flex-col gap-y-6'>
      ...
      <div className='grid grid-cols-2 gap-6'>
        {statusCounts.filter((s) => s.count !== 0).map((status) => {
          ...
          const isDelivered = status.id === requestStatuses.delivered
          const isCancelled = status.id === requestStatuses.requestCancelledDuplicateRequest
