...
const QuantityStep = ({ currentStep, onQuantitySubmit }: { currentStep: number, onQuantitySubmit: (quantity: number) => void }) => {
  ...
  return (
    <div className='grid grid-cols-3 gap-2'>
      <div className='flex justify-between col-span-3 p-4 bg-gray-100 rounded-lg'>
      ...
      <button onClick={handleSubmit} className='flex items-center hover:cursor-pointer justify-center bg-emerald-100 rounded-lg p-4 font-semibold text-2xl'>
        Submit
      </button>
    </div>
  );
