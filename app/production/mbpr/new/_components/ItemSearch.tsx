  ...
  return (
    <div>
      <input
        ...
        className="w-full input py-2 px-4 rounded-lg  input-primary bg-primary/50 text-primary-content text-lg mb-6"
      />

      <ul>
        <div className="flex flex-col gap-y-4 overflow-y-auto max-h-[600px]">
          {results.map((item) => (
            <li className="bg-primary/30 rounded-lg px-4 py-2" key={item.id} onClick={() => handleItemClick(item)}>
