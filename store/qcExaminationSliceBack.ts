...
export const useQcExaminationSelection = create<State & Actions>((set, get) => ({
  ...
  actions: {
    ...
    setExaminationRecordId: async () => {
      ...
      const bpr = lot.lotOrigin && lot.lotOrigin.bprId
      const po = lot.lotOrigin && lot.lotOrigin.purchaseOrderId
      ...
      const record = await qualityActions.qc.records.create({
        ...
        ...(lot.lotOrigin && lot.lotOrigin.originType === 'batchProduction' ? { linkedBprId: bpr } : { linkedPurchaseOrderItemId: po }),
      })
