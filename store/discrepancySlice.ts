...
type Actions = {
  actions: {
    setSelectedItem: (id: string, fromLot?: boolean) => void;
...
export const useDiscrepancySelection = create<State & Actions>((set, get) => ({
  mode: 'item' as DiscrepancyAppMode,
  audit: null,
  auditItems: [],
  item: null,
  itemLots: [],
  ...
  actions: {
    clearSelectedItem: () => set(() => ({ item: null })),
    setSelectedItem: async (id, fromLot = true) => {

      set(() => ({ isItemLoading: true }));
      try {
        const itemId = fromLot ? await getItemIdFromLot(id) : id;
